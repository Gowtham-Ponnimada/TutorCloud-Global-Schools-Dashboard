...
echo "==> Preparing 2024-2025 directory-only US setup"
mkdir -p "$REPO/data/us/raw/2024_2025_directory"
mkdir -p "$REPO/data/us/extracted/2024_2025_directory"
...
cat > "$REPO/sql/us/01_us_directory_2024_2025_schema.sql" <<'SQLEOF'
...
cat > "$REPO/sql/us/02_us_directory_2024_2025_views.sql" <<'SQLEOF'
...
cat > "$REPO/scripts/us/load_us_directory_2024_2025_only.py" <<'PYEOF'
...
RAW_DIR = REPO / "data/us/raw/2024_2025_directory"
EXT_DIR = REPO / "data/us/extracted/2024_2025_directory"
...
def extract_zip(zip_path: Path, out_dir: Path):
    ...
    marker.write_text(datetime.now().astimezone().isoformat(), encoding="utf-8")
...
def main():
    run_sql_file(SQL_DIR / "01_us_directory_2024_2025_schema.sql")

    download(URLS["schools_2425"], RAW_DIR / "ccd_sch_029_2425_w_0a_051425.zip")
    download(URLS["schools_2425_docs"], RAW_DIR / "2024-25_Sch_Documentation_prelim.zip")
    download(URLS["lea_2425"], RAW_DIR / "ccd_lea_029_2425_w_0a_051425.zip")
    download(URLS["lea_2425_docs"], RAW_DIR / "2024-25_Lea_Documentation_prelim.zip")

    for z in RAW_DIR.glob("*.zip"):
        extract_zip(z, EXT_DIR / z.stem)

    school_flat = choose_flat_file(EXT_DIR / "ccd_sch_029_2425_w_0a_051425")
    lea_flat = choose_flat_file(EXT_DIR / "ccd_lea_029_2425_w_0a_051425")

    if school_flat is None:
        raise RuntimeError("No 2024-25 school flat file found after extraction.")
    if lea_flat is None:
        raise RuntimeError("No 2024-25 LEA flat file found after extraction.")

    schools_raw = read_any(school_flat)
    lea_raw = read_any(lea_flat)

    to_sql(schools_raw, "stg_schools_2024_2025_raw")
    to_sql(lea_raw, "stg_lea_2024_2025_raw")
    ...
    dim_states["created_at"] = datetime.now().astimezone()
    ...
    run_sql_file(SQL_DIR / "02_us_directory_2024_2025_views.sql")

    report = {
        "status": "success",
        "loaded_at_utc": datetime.now().astimezone().isoformat(),
    ...
    out = REPORT_DIR / f"us_directory_2024_2025_load_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
cat > "$REPO/scripts/us/validate_us_directory_2024_2025.py" <<'PYEOF'
from __future__ import annotations
...
"""
US dashboard renderer placeholder for 2024-2025 directory-only mode.
Next step is to generate the full dashboard scaffold against us.dim_states,
us.dim_districts, and us.dim_schools.
"""

def render_us_home():
    raise NotImplementedError("US directory-only 2024-2025 data layer is ready. Renderer scaffold pending.")
...
"""
Placeholder routing patch file.
...
echo "==> Syntax check"
"$PY" -m py_compile \
  "$REPO/scripts/us/load_us_directory_2024_2025_only.py" \
  "$REPO/scripts/us/validate_us_directory_2024_2025.py" \
  "$REPO/utils/us_page_renderer.py" \
  "$REPO/scripts/us/patch_us_routing.py"

echo "==> Running 2024-2025 directory-only US loader"
"$PY" "$REPO/scripts/us/load_us_directory_2024_2025_only.py" | tee /tmp/us_directory_2024_2025_load.log

echo "==> Running validator"
"$PY" "$REPO/scripts/us/validate_us_directory_2024_2025.py" | tee /tmp/us_directory_2024_2025_validate.log

echo "==> Latest US reports"
ls -1dt "$REPO"/reports/us/* | head -10

echo "==> 2024-2025 directory-only US setup completed"
