...
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    uae_section_header(f"\U0001F4CA Enrollment Overview \u2014 {sel_year}")
        ...
        if not enr_emirate.empty:
            st.plotly_chart(
                bar_h(enr_emirate, x="total_students", y="region_en",
                      title=f"\U0001F4CD Students Enrolled by Emirate ({sel_year})",
...
# ══════════════════════════════════════════════════════════════════════════════
with tab2:
    uae_section_header(f"\U0001F3EB Schools \u2014 {sel_year}")
        ...
        if not sch_emirate.empty:
            st.plotly_chart(
                bar_h(sch_emirate, "total_schools", "region_en",
                      f"\U0001F3EB Schools by Emirate ({sel_year})",
...
# ══════════════════════════════════════════════════════════════════════════════
with tab3:
    uae_section_header(f"\U0001F468\u200D\U0001F3EB Teachers & Staff \u2014 {sel_year}")
        ...
        if not tch_yr.empty:
            st.plotly_chart(
                bar_h(tch_yr, "total_teachers", "region_en",
                      f"\U0001F468\u200D\U0001F3EB Teachers by Emirate ({sel_year})",
                      color_scale=["#bbdefb", UAE["blue"]], height=400),
                use_container_width=True,
            )
    with col_tb:
        ...
        if not tsub.empty:
            st.plotly_chart(
                bar_h(tsub.head(15), "total_teachers", "subject_en",
                      f"\U0001F4D6 Top Subjects by Teacher Count ({sel_year})",
...
# ══════════════════════════════════════════════════════════════════════════════
with tab4:
    uae_section_header(f"\U0001F4C8 Academic Performance \u2014 {sel_year}")
        ...
        if not pf_cycle.empty:
            ...
            st.plotly_chart(
                perf_bar(cycle_summary, "avg_pass_rate", "cycle",
                         f"\U0001F4DA Pass Rate by Cycle ({sel_year})",
                         range_color=(70, 100), height=400),
                use_container_width=True,
            )
        ...
        if not sub_scores.empty:
            st.plotly_chart(
                perf_bar(sub_scores.head(15), "avg_pass_rate", "subject_en",
                         f"\U0001F4D6 Pass Rate by Subject ({sel_year})",
        ...
        if not top20.empty:
            st.plotly_chart(
                bar_h(top20, "student_count", "nationality_en",
                      f"\U0001F30D Top 20 Student Nationalities ({nat_yr_sel})",
